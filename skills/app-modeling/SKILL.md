...
- Database credentials follow the type's schema: if it defines `username`/`password`, set them on the resource; if it accepts a Secret resource, create a `Radius.Security/secrets` and reference it; if it defines neither, the type takes no credentials. Always use a `@secure() param` for the password. When a workload consumes user-authored credentials through Radius connection environment variables, connect the authored Secret with `source: <secret>.id`.
...
- For a user-authored input Secret, connect the `Radius.Security/secrets` resource directly with `source: <secret>.id`.
- For Recipe output credentials, connect only the producer with `source: <producer>.id`. Radius projects each declared secret output as a secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variable; `<SECRETKEY>` comes from the exact Recipe output key, not from a guessed generic suffix. Do not add a second connection to the managed Secret.
...
- **Inputs** (credentials you supply): follow the exact schema — set an `x-radius-sensitive` property (e.g. `password`) from a `@secure()` parameter; author `Radius.Security/secrets` when the schema or workload connection contract accepts a Secret resource; or supply none. To project a user-authored Secret into a workload, connect it with `source: <secret>.id`. Do not confuse this input Secret with a Recipe-owned output Secret.
- **Outputs** (values a Recipe generates): inspect the exact registered schema and Recipe output mapping. Connect only to the producer with `source: <producer>.id`; each declared secret output is injected as a secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variable. The suffix is derived from the exact Recipe output key. Use `secretName: <producer>.properties.secrets.name` plus that declared key only when the application explicitly requires a custom Kubernetes environment-variable name through `secretKeyRef`. Never connect to or author a wrapper for the Recipe-owned Secret.
...
- [ ] A user-authored input credential is carried by `Radius.Security/secrets` and connected with `source: <secret>.id` when the workload consumes connection projection. Recipe-generated credentials use only the producer connection and the exact secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` name derived from the Recipe output key. A custom native environment name uses `secretKeyRef` with `<producer>.properties.secrets.name` and the declared key. Explicit `env` wins on a duplicate name; use `disableDefaultEnvVars` only when the schema supports it and all generated variables for that connection must be suppressed. No authored secret copies an output, guesses a suffix, or interpolates an aggregate credential-bearing URL/config.
