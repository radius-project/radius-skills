...
8. Generate the Bicep using [naming-conventions.md](references/naming-conventions.md), then run `node "<loaded-skill-base>/scripts/validate-bicep.mjs" .radius/app.bicep`. The checker uses only the extension-managed Bicep. Repair every compiler error and warning until the checker exits successfully and prints no warnings. Never make compilation pass by deleting a required backend activation, native configuration value, secret binding, or dependency edge.
...
- Database credentials follow the type's schema: if it defines `username`/`password`, set them on the resource; if it accepts a Secret resource, create a `Radius.Security/secrets` and reference it; if it defines neither, the type takes no credentials. Always use a `@secure() param` for the password. Author or reuse a workload-facing Secret only when the workload needs a Secret connection or the resource schema requires one.
...
- Explicit native variables may coexist with generic projection. For one generated name, explicit `env` takes precedence, then a managed secret reference, then an ordinary property value. Reject secret-backed names that collide after uppercasing. Set `disableDefaultEnvVars: true` on the connection only when all of that connection's generated variables must be suppressed; never set it when the workload relies on a generated secret-backed variable.

## Secrets

See [secrets-handling.md](references/secrets-handling.md). Secret contracts are version- and type-specific:

- **Inputs** (credentials you supply): follow the exact schema — set an `x-radius-sensitive` property (e.g. `password`) from a `@secure()` parameter; author or reuse `Radius.Security/secrets` only when the schema requires a Secret or the workload consumes a Secret connection; or supply none. Connect a workload-facing authored Secret with `source: <secret>.id`. Preserve an existing explicit `env`, `secretKeyRef`, `envFrom`, or native-variable binding when projection compatibility is unproven or the application needs a different name.
- **Outputs** (values a Recipe generates): inspect the exact registered schema and Recipe. A wrapped Recipe declares generated credentials under `result.secrets`; connect only to the producer with `source: <producer>.id`. Each declared secret output is injected by a compatible Kubernetes Container Recipe as a secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variable whose suffix follows the exact key. Use `secretName: <producer>.properties.secrets.name` plus that key only when the application explicitly requires a custom Kubernetes environment-variable name through `secretKeyRef`. Never connect to or author a wrapper for the Recipe-owned Secret.
...
- [ ] Every planned resource property read/write has its verbatim path in the ledger and exists in the exact target schema/API version. Every Recipe-generated credential has an exact `result.secrets` key and mapping copied from the target Environment Recipe or matching immutable provider recipe source; every explicit custom `secretKeyRef` has the declared nested secret-name path and key; every omitted optional Recipe input is proven safe. An absent path blocks generation rather than being replaced by a guessed property, alias, wrapper, or stale mutable-extension shape.
...
- [ ] `node "<loaded-skill-base>/scripts/validate-bicep.mjs" .radius/app.bicep` exits successfully and prints no errors or warnings. The file compiles with an extension compatible with the target Environment schema/Recipe contract; every `Radius.*` type is on the allow-list or is a generated `Radius.Resources/*` custom type, and matches that version's schema and API version. A conflicting mutable extension is version drift, not permission to alter required deployment wiring.
...
- [ ] Kubernetes secret-backed projection is used only after proving compatible Radius and Container Recipe support; ACI and older/unknown installations retain safe explicit wiring. Developer-owned credentials use an authored/reused Secret only when needed and connect through `<secret>.id`; Recipe-generated credentials use only `<producer>.id` and exact `result.secrets` keys. A custom native environment name uses `secretKeyRef` with `<producer>.properties.secrets.name`. Precedence is explicit `env`, managed secret reference, then ordinary property; secret-backed names that collide after uppercasing fail validation. `disableDefaultEnvVars` suppresses all variables for its connection and is absent when one is required. Existing working models are not automatically migrated.
