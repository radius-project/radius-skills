...
Before writing Bicep, enumerate every planned resource property read and write as a separate ledger row. Record the verbatim path and prove it against the exact target schema and API version. For a generated output, open the exact target Environment Recipe or matching immutable provider recipe-pack source and record the verbatim output mapping; schema prose, property names, and READMEs do not prove that a deployed Recipe returns a value. A wrapped Recipe returns generated credentials under `result.secrets`; keep that result shape distinct from the recipe pack's module-output mapping. Prove the exact key that determines each secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` suffix and prove compatible Kubernetes Container Recipe support before relying on projection. Prove `<producer>.properties.secrets.name` and the declared key only when a custom native Kubernetes environment-variable name is required. Any row that connects the managed Secret separately, copies it into an authored secret, relies on projection for ACI/unknown contracts, or introduces colliding secret-backed names after uppercasing fails preflight.
...
- a connection to a user-authored input `Radius.Security/secrets` via `<secret>.id`;
- a connection to a producer via `<producer>.id`, including secret-backed generated variables for exact Recipe-declared secret output keys;
- `valueFrom.secretKeyRef` via `<producer>.properties.secrets.name` and a declared key only for an explicitly required custom Kubernetes environment-variable name;
...
| Secret | A user-authored input Secret connection, a producer connection's exact secret-backed Recipe output variable, or an explicit custom native binding through `<producer>.properties.secrets.name` and the declared key |
...
4. Confirm authored/reused input Secrets exist only when needed and connect through `.id`; Recipe-generated credentials come from the producer connection's exact `result.secrets` keys; and custom native bindings use `<producer>.properties.secrets.name`. Prove Kubernetes projection compatibility and retain safe explicit/native fallbacks otherwise. Apply precedence in this order: explicit `env`, managed secret reference, ordinary property; reject colliding secret-backed names after uppercasing. Ensure `disableDefaultEnvVars` does not suppress required values. Do not automatically migrate an existing working model.
